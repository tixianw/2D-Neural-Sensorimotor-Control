...
class Environment(ArmEnvironment):
	...
	def set_target(self):
		if not self.flag_shooting:
			x_star = 0.16 / 0.2 * self.arm_param['L']
			y_star = 0.16 / 0.2 * self.arm_param['L']
		else:
			x_star = 0.10 / 0.2 * self.arm_param['L']
			y_star = 0.12 / 0.2 * self.arm_param['L']
