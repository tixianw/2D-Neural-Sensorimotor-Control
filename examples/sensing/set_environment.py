...
class Environment(ArmEnvironment):
	...
	def set_sensors(self):
		n_sensor = 21 # 21 ## number of sensors (first one locate at the origin)
		...
		l_mu = 0.5 # 0.9 # 
		u_mu = 1.5 # 1.1 # 
		mu0 = np.array([np.random.uniform(self.diffusion_simulator.mu*l_mu, self.diffusion_simulator.mu*u_mu) for i in range(n_sensor)]) # np.ones([n_sensor]) * self.diffusion_param['mu_true'] # 
		theta0 = np.hstack([0, d2r(np.random.randn(n_sensor-1)*10)]) # 18 # np.zeros_like(alpha0)
		# theta0 = np.hstack([0, d2r(np.array([np.random.randint(360)-180 for i in range(n_sensor-1)]) * 0.1)]) + theta_true[::self.sensor_skip]
		gain_mu = 4e4 # 4e4 # 
		gain_r = 4e4 # 4e4 # 
		gain_theta = 5e4 # 1e4 # 1e5 # 
		neurons_chemo = None
